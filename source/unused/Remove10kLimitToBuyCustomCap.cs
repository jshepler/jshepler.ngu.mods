...
namespace jshepler.ngu.mods
{
    ...
    internal class Remove10kLimitToBuyCustomCap
    {
        ...
        [HarmonyTranspiler, HarmonyPatch(typeof(Resource3Purchases), "updateCustomCapInput")]
        private static IEnumerable<CodeInstruction> Resource3Purchases_updateCustomCapInput_transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var cm = new CodeMatcher(instructions)
                .MatchForward(false, new CodeMatch(new CodeInstruction(OpCodes.Ldc_I4, 10000)))
                .Advance(-5);
